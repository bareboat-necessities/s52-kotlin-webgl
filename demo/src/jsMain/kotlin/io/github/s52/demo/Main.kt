...
fun main() {
    ...
    val presLib = PresLibPack.phase0Minimal()
    val settings = MarinerSettings()
    val context = PortrayalContext(compilationScale = 50_000.0, displayScale = 50_000.0)
    val engine = S52PortrayalEngine(presLib.lookupTable, DefaultCspRegistry.phase0())
    val commands = engine.portray(phase0SyntheticFeatures(), settings, context)
    val stats = WebGlS52Renderer(canvas, presLib).render(commands, settings)

    status.textContent = "Generated ${commands.size} draw commands: $stats"
}

private fun phase0SyntheticFeatures(): List<EncFeature> = listOf(
    EncFeature(
        id = 1,
        ...
        geometry = EncGeometry.Polygon(
            outer = listOf(
                ...
                Coordinate(-73.90, 40.00),
                Coordinate(-73.90, 40.10),
                Coordinate(-74.00, 40.00)
            )
        )
    ),
    EncFeature(
        ...
        geometry = EncGeometry.Polygon(
            outer = listOf(
                ...
                Coordinate(-73.80, 39.90),
                Coordinate(-73.80, 40.20),
                Coordinate(-74.10, 39.90)
            )
        )
    ),

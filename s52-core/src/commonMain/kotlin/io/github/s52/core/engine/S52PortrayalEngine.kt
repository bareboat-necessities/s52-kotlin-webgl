...
class S52PortrayalEngine(
    ...
    fun portray(
        ...
        return features
            ...
            .filter { isVisibleInDisplayCategory(it.category, settings.displayCategory) }
    ...
    fun portrayFeature(
        ...
        return lookupTable.match(feature, settings, context).flatMap { record ->
            expandInstructions(record.instructions, feature, settings, context)
                .mapNotNull { instruction -> instruction.toDrawCommand(feature, record) }
        }
    }

    private fun expandInstructions(
        instructions: List<S52Instruction>,
        feature: EncFeature,
        settings: MarinerSettings,
        context: PortrayalContext
    ): List<S52Instruction> {
    ...
    private fun isVisibleInDisplayCategory(
        commandCategory: DisplayCategory,
        selectedCategory: DisplayCategory
    ): Boolean {
        return when (selectedCategory) {
            DisplayCategory.DisplayBase -> commandCategory == DisplayCategory.DisplayBase
            DisplayCategory.Standard -> commandCategory == DisplayCategory.DisplayBase || commandCategory == DisplayCategory.Standard
            DisplayCategory.Other,
            DisplayCategory.MarinersStandard -> true
        }
    }
}
